...
import 'object_detection/any_object_detection_controller.dart';

final findWayCameraController = Provider(FindWayCameraController.new);

class FindWayCameraController {
  FindWayCameraController(this.ref);
  ...
  Future<void> startImageStream(int cameraIndex) async {
    await ref.read(anyObjectDetectionController.notifier).initialize();
    ...
    await _cameraController!.startImageStream((image) async {
      ...
      try {
        await ref.read(anyObjectDetectionController.notifier).detectObject(image);
      } finally {
        _isProcessing = false;
      }
    });
