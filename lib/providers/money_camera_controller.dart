...
import 'package:nguoi_khuyet_tat/providers/money_detection/money_detection_controller.dart';

final moneyCameraController = Provider(MoneyCameraController.new);

class MoneyCameraController {
  MoneyCameraController(this.ref);
  ...
  Future<void> startImageStream(int cameraIndex) async {
    await ref.read(moneyDetectController.notifier).initialize();
    ...
    await _cameraController!.startImageStream((image) async {
      ...
      try {
        await ref.read(moneyDetectController.notifier).detectMoney(image);
      } finally {
        _isProcessing = false;
      }
    });
