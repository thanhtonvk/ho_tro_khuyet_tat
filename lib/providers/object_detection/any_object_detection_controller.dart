...
final anyObjectDetectionController =
    StateNotifierProvider<AnyObjectDetectionController, List<YoloResult>>(
  AnyObjectDetectionController.new,
);

class AnyObjectDetectionController extends StateNotifier<List<YoloResult>> {
  AnyObjectDetectionController(this.ref) : super([]);
  ...
  Future<void> initialize() async {
    ...
    flutterTts.setCompletionHandler(() async {
      await Future.delayed(const Duration(seconds: 5)); // Đợi 3 giây
    ...
    switch (cameraImage.format.group) {
      ...
      case ImageFormatGroup.yuv420:
        state = nguoiKhuyetTatSDK
            .detectAnyObjectYUV420(
              y: cameraImage.planes[0].bytes,
              u: cameraImage.planes[1].bytes,
              v: cameraImage.planes[2].bytes,
              height: cameraImage.height,
              deviceOrientationType:
                  ref.read(findWayCameraController).deviceOrientationType,
              sensorOrientation:
                  ref.read(findWayCameraController).sensorOrientation,
              onDecodeImage: (image) {
        ...
        if (flash == 'bright') {
          ref.read(findWayCameraController).toggleFlash('bright');
        } else {
          ref.read(findWayCameraController).toggleFlash('dark');
        }
        ...
        if (state.isNotEmpty) {
          ...
          double y = obj.y;
          if (x < width / 2) {
            _speak("Có vật cản bên trái");
          } else {
            _speak("Có vật cản bên phải");
          }
        }
        break;
